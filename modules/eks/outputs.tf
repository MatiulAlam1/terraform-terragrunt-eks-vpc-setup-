...
output "node_ssh_security_group_id" {
  description = "Security group ID for SSH access to EKS worker nodes"
  value       = aws_security_group.eks_node_ssh.id
}

output "sample_app_url" {
  description = "Instructions for accessing the sample application"
  value       = "After deployment, run the setup script: ./setup-eks-access.ps1"
}
